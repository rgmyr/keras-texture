...
class BirdsDataset(Dataset):
    ...
    def load_or_generate_data(self):
        ...
        self.X_train = np.array([center_crop(io.imread(self.img_paths[i]), input_size) for i in self.train_ix])
        self.y_train = [self.img_labels[i] for i in self.train_ix]

        self.X_test = np.array([center_crop(io.imread(self.img_paths[i]), input_size) for i in self.test_ix])
        self.y_test = [self.img_labels[i] for i in self.test_ix]
    ...
    def __repr__(self):
            return (
                'Birds-200 Dataset\n'
                f'Num images: {len(self.train_ix)}/{len(self.test_ix)}
